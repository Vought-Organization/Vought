...
export const useServices = () => {
  const users = axios.create({
    baseURL: 'http://backend-vought.duckdns.org:8080/v1/users',
    headers: { 'Content-Type': 'application/json' },
  });

  const tickets = axios.create({
    baseURL: 'http://backend-vought.duckdns.org:8080/v1/tickets',
  });

  const events = axios.create({
    baseURL: 'http://backend-vought.duckdns.org:8080/v1/events',
    headers: { 'Content-Type': 'application/json' },
  });
